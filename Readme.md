...
- データ放送を削除してさらに圧縮する
  ...
  ```--smart-remove-typed```を指定すると、先頭60秒、中間点の前後30秒、末尾60秒ではデータ放送を保持し、それ以外の区間のみ削除します。置き換え映像を指定せず、```-i```と```-o```だけでType-Dのtrimを行うこともできます。
...
入力TSの先頭60秒、中間点の前後30秒、末尾60秒ではISO/IEC 13818-6 type Dを保持し、それ以外の区間から削除します。3つの保持区間が重なる短いTSでは、重なった区間をまとめて保持します。

入力時間と中間点を確定する必要があるため、標準入力には対応せず、時間を取得できる通常の入力ファイルが必要です。映像を置き換えずにtrimのみを行う場合は次のように実行します。
