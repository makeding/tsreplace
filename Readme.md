...
- データ放送を削除してさらに圧縮する
  ...
  ```--smart-remove-typed```を指定すると、14.5分ごとに1分間データ放送を保持し、それ以外の区間のみ削除します。予定時間を渡した場合は末尾1分間も保持します。置き換え映像を指定せず、```-i```と```-o```だけでType-Dのtrimを行うこともできます。
...
入力TSの先頭から14.5分ごとに1分間、ISO/IEC 13818-6 type Dを保持し、それ以外の区間から削除します。この周期を入力のEOFまで繰り返すため、事前に入力時間を取得する必要はありません。
